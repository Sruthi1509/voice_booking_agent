...
- **Phonetic Location & Item STT Homophone Correction**: Resolves speech-to-text variations and regional homophones for locations (e.g., *"Vizianagaram"* / *"Vijayanagaram"* $\rightarrow$ *"Vijayanagar"*) and items (e.g., *"tails"*, *"tiles"*, *"tales"*, *"share"* $\rightarrow$ *"chairs"*; *"bridge"* $\rightarrow$ *"fridge"*). If a location is outside the direct service zone, the agent suggests nearby served areas.
- **Generalized Non-Transportable & Illogical Item Filtering**: Automatically detects non-transportable categories (live animals/pets, passengers/people, hazardous/flammable materials, non-physical elements) and unrecognized objects. Explicitly rejects non-transportable items and prompts the user to confirm or re-state their item request whenever an ambiguous or doubtful item is heard.
- **Item Quantity & Logic Verification**: When big items (like chairs, tables, sofas, fridges, beds) are named without specifying count/quantity (e.g., *"chairs"*), the assistant explicitly prompts for the number of items because item count determines the accurate vehicle size (Two-wheeler, Mini-van, Mini-truck).
